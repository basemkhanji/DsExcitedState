...
def RunMyJobsOnThreads( PATH_TO_File_list ):
    print '------------------------------------------------------------------'
    print 'Running my jobs now on diffenet threads'
    print 'Worker is acquired now:'
    for thrd_file in PATH_TO_File_list:
        t = threading.Thread(target= RunMyJobs , args =( thrd_file  , ) )
        t.start()
    print 'Worker is released now .... moving to the next task!'
    print '------------------------------------------------------------------'
...
iter_my_list  = iter(files_list)
for Root_files in iter_my_list:
    i_file +=1
    Colorprint( ' File  ' + str(i_file) + ' out of ' + str(len(files_list) ) , 'green')
    #pool.apply_async( RunMyJobs , ( Root_files , ) )
    Root_files_l = [ Root_files , next(iter_my_list)]
    print Root_files_l 
    pool.apply_async( RunMyJobsOnThreads , ( Root_files_l , ) )
